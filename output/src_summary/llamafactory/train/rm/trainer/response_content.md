这个脚本的核心功能是定义了一个名为`PairwiseTrainer`的类，它是`transformers`库中的`Trainer`类的子类。这个子类主要用于对特定类型的模型（可能是用于比较两个输入的模型，如排名或选择模型）进行微调。它扩展了基本的训练功能，包括自定义优化器、学习率调度器、损失计算和预测结果的保存。
