这个脚本的核心功能是管理一个训练或评估（称为“运行”）的过程，该过程基于Hugging Face的Transformers库进行模型的微调或评估。它通过一个图形用户界面（可能使用Gradio库实现）接收用户输入，这些输入包括模型参数、训练配置等。脚本根据用户输入生成训练或评估命令，然后启动命令并监控其运行状态。
